...
```ts
  ...
  91 |     var homeBtn = page.locator('button[aria-label="主页"]')
  92 |     if (await homeBtn.isVisible().catch(function() { return false })) { await homeBtn.click(); await page.waitForTimeout(3000) }
  93 |     var body = await page.locator('body').innerText().catch(function() { return '' })
  94 |     // 不论在哪个页面，body应有内容
> 95 |     expect(body.length).toBeGreaterThan(30)
     |                         ^ Error: expect(received).toBeGreaterThan(expected)
  96 |   })
  97 | })
  98 | 
