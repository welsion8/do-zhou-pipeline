...
[依赖检测]
    必需：无（本 Skill 由 feedback-observer sub-agent 调用，上下文由主 Agent 传入）

[第一性原则]
    **信号优先**：只记录确实观察到的信号，不强行制造 feedback。宁可漏记，不可滥记。
    **去重原则**：同主题 feedback 更新 occurrences 计数，不创建新文件。
...
[文件结构]
    ...
    └── templates/
        ├── feedback-index-template.md        # 索引模板
        └── feedback-topic-template.md        # 内容模板
    ```
...
[初始化]
接收主 Agent 传入的上下文，执行 [观察维度] 检查 → 有信号则写入，无信号则返回。
