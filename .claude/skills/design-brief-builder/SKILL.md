...
[工作流程]
    ...
    [输出阶段]
        ...
        第三步：Spec→Design 覆盖验证（硬性门禁）
            对照 Product-Spec.md 的功能清单，逐项检查 Design-Brief 是否提供了对应设计描述：
            1. 提取 Spec 中所有 UI 相关功能点（页面、弹窗、面板、状态、交互组件）
            2. 逐项在 Design-Brief 中查找对应的视觉描述或设计方向
            3. 输出覆盖矩阵：`| Spec功能 | Design-Brief覆盖 | 状态 |`
            4. 发现未覆盖项 → 补充设计描述后再输出
            5. 覆盖率达到 100% 才能进入下一步
            - 注意：此时可能还没有详细设计帧（.pen），Design-Brief 阶段要求的是"设计方向已定义"，不是"像素已到位"
