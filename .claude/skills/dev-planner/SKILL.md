...
[第一性原则]
    ...
    **基本可操作性原则**：骨架 Phase（项目初始化后的第一个 Phase）交付时，窗口/页面必须可正常操作，不依赖后续 Phase。最低标准：
    - Desktop（Electron）应用：窗口可拖拽移动 + 有关闭/最小化/最大化按钮
    - Web 应用：页面可正常加载 + 浏览器 Tab 标题正确 + 无白屏
    - CLI 应用：命令可执行 + --help 正常输出
    不允许出现"窗口关不掉"、"页面白屏"、"命令无响应"等基本操作缺失。

    **最小可用入口原则**：任何服务层 Phase（数据库、文件系统、API、引擎）交付时，必须附带一个能调用该服务的最小 UI 入口——哪怕只是一个按钮、一行命令、一个测试页面。不允许出现"服务已就绪但后续 3 个 Phase 都无法验证它"的情况。
    - 判据：下一个 Phase 开工时，能否用上一个 Phase 的真实数据做开发验证？如果不能，上一个 Phase 缺少最小可用入口。
    - 入口不要求完整 UI：临时按钮、内联表单、dev 模式下的快捷操作均可。关键是让后续 Phase 有真数据可跑，不靠 mock。
    - 例：文件系统 Phase → 附带"创建示例项目"按钮，让编辑器 Phase 有真实文件可打开
    - 例：API 配置 Phase → 配置完成后工具栏切换下拉立即可用，不等到 AI 对话 Phase 才接上
...
[分析策略]
    ...
    **设计帧映射法（有设计稿时必须执行，不可跳过）**
    如有设计工具 MCP 已连接，在 Phase 拆分后逐帧检查覆盖：
    1. 通过设计工具读取所有设计帧 ID 和名称
    2. 列出完整的帧清单：`| 帧ID | 帧名称 | 对应 Phase | 对应 Task |`
    3. 确保每一帧都有对应的 Phase Task 承接其实现
    4. 未被任何 Phase 覆盖的设计帧 → 必须补充 Phase 或扩展现有 Phase 的交付清单
    5. 未被任何设计帧覆盖的 Spec 功能 → 标记 [待设计补充]，在 DEV-PLAN 中标注
    6. **反向覆盖强制门禁（本次新增）**：每个含 UI 交付物的 Phase，必须有至少一个设计帧支撑。逐 Phase 检查：
       - Phase 标记为"无设计帧"且不是基础设施/后端引擎 Phase → 🔴 阻断，该 Phase 不得进入开发
       - 基础设施 Phase（Electron 骨架、数据库初始化）和后端引擎 Phase → 豁免，标注"无 UI"
       - 输出反向覆盖矩阵：`| Phase | UI交付物 | 对应设计帧 | 状态 |`
    7. 输出覆盖矩阵作为 DEV-PLAN 的附件或内嵌表格
    - 注意：反向覆盖检查是防止 Agent 凭空创造设计稿中不存在的界面。没有设计帧 = 不能写 UI 代码 = Phase 不允许有 UI 交付物
...
[信息充足度判断]
    ...
    **必须满足**（没有这些，Plan 是废纸）：
    ...
    - ✅ 如有设计稿：所有设计帧都有对应的 Phase Task 承接（设计帧→Task 覆盖矩阵 100%）
    - ✅ 如有设计稿：所有 Spec 功能都有对应的设计帧覆盖（Spec→设计帧 逆映射）
...
[工作流程（生成模式）]
    ...
    [输出阶段]
        ...
        第一步：加载模板
            ...
            如模板文件不存在 → 报告缺失，使用内置默认结构继续。不阻塞输出
        ...
        第三步：自检
            ...
            运行审计预扫描：`node .claude/audit-runner/audit-pipeline.js` — 检查 D4 引用有效性（Plan 中列出的文件路径是否合理）和 D1 覆盖完整性（Spec 功能→Phase 映射是否有遗漏）
